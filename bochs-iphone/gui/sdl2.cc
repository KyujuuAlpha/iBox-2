...
bx_sdl2_gui_c::bx_sdl2_gui_c()
{
  ...
  if (SDL_Init(flags) < 0) {
    ...
    panic("Unable to initialize SDL2 libraries: %s", SDL_GetError());
    return;
  }
  atexit(SDL_Quit);
...
void bx_sdl2_gui_c::specific_init(int argc, char **argv, unsigned tx, unsigned ty, unsigned headerbar_y)
{
  int i, j;
  unsigned icon_id;
#ifdef WIN32
  ...
  if (window == NULL) {
    ...
    BX_PANIC(("Unable to create SDL2 window: %s", SDL_GetError()));
    return;
  }
